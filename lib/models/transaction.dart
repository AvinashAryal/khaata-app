...
class Record{
  ...
  Record({required this.transactionID, required this.borrowerID, required this.lenderID, required this.transactionDate, required this.amount}) ;

  // Convert tuple to JSON so that we can later push it to the cloud store
  Map<String, dynamic> toJSON() => {
    'transactionID' : transactionID, 'borrowerID' : borrowerID, 'lenderID' : lenderID, 'transactionDate': transactionDate, 'amount' : amount
  } ;
  ...
  static Record fromJSON(Map<String, dynamic> json) =>
      Record(transactionID: json['transactionID'], borrowerID : json['borrowerID'],
                  lenderID : json['lenderID'], transactionDate: json['transactionDate'], amount: json['amount']) ;
